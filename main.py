...
def drawWindow(letter_list, game_word, alphabet_list, game_over, key_pressed=-1):
    ...
    # the starting x and y positions for the dashed line representing the word
    x = 400
    y = 300

    # drawing the dashed line representing the word
    for i in range(len(game_word)):
    ...
    if win_state != "Neither":
        ...
        win.blit(text, (300 + x_change, 200 + y_change))
        ...
        word_length, word_height = font.size(to_display)
        x_change = int((200 - word_length) / 2)
        win.blit(text, (300 + x_change, 200 + y_change + 30))
        return True
    else:
        return False
...
while run:
    ...
    for event in pygame.event.get():
        ...
        # checking if the user has clicked the replay button
